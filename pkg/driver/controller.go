...
func waitForReadySnapshot(ctx context.Context, snapname string) error {
	for {
		...
		case zfs.ZFSStatusFailed:
			return zfs.FailureFromEvent(ctx, "ZFSSnapshot", snap.Name, snap.Namespace, snap.UID,
				fmt.Sprintf("snapshot %s creation failed on node %s", snapname, snap.Spec.OwnerNodeID))
		}
